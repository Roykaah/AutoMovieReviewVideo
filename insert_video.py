...
def initialize_upload(youtube, options):
    ...
    body = dict(snippet=dict(
        title=f"{movie_splited[0]} - ({movie_splited[1]}) Speedy Movie Review",
    ...
    videoPath = os.path.join(IMAGES_DIR, 'final_video.mp4')
    ...
    while response is None:
        try:
            ...
            if response is not None:
                if 'id' in response:
                    print(
                        'The video with the id %s was successfully uploaded!' %
                        response['id'])
